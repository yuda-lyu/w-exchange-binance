...
let cleanupByResult = async (r) => {
    ...
    for (let algoId of [r.algoIdTp, r.algoIdSl]) {
        ...
        try {
            await webina.opBinaContractCancelAlgoOrder(st, Number(algoId), { forceTest: true })
        }
        catch (e) {
            //已成交/已撤則略過
        }
    ...
    try {
        ...
        if (qty > 0) {
            ...
            await webina.opBinaContractMarketEntry(st, SYMBOL, side, positionSide, qty, { forceTest: true })
        }
    }
    catch (e) {
        //平倉失敗不中斷(避免影響其他測試清理)
...
let resetTestAccount = async () => {
    ...
    //撤所有普通單
    try {
        await webina.opBinaContractCancelAllOrder(st, { forceTest: true })
    }
    catch (e) {
    ...
    //撤所有algo單
    try {
        await webina.opBinaContractCancelAllAlgoOrder(st, { forceTest: true })
    }
    catch (e) {
    ...
    try {
        let poss = await webina.opBinaContractQueryPositionInfor(st, { forceTest: true })
        for (let p of (poss || [])) {
            ...
            await webina.opBinaContractMarketEntry(st, SYMBOL, side, positionSide, Math.abs(amt), { forceTest: true })
        }
    }
    catch (e) {
